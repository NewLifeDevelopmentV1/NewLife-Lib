...
function nl_lib.GetPlayerGangInfo(source) end -- * Not supported in ox_core natively - would require gangs to be set up with a specific type in DB
...
function nl_lib.AddItem(source, item, count, metadata)
    if not ox_inventory then return false end
    return exports.ox_inventory:AddItem(source, item, count, metadata)
end

function nl_lib.HasItem(source, item)
    if not ox_inventory then return false end
...
function nl_lib.RegisterUsableItem(_item, cb)
    if not ox_inventory then return false end
